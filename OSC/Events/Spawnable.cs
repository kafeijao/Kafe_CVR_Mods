...
public static class Spawnable
{
    ...
    internal static void OnSpawnableCreated(CVRSyncHelper.PropData propData)
    {
        ...
        PropCache.TryAdd(propData.InstanceId, propData);
    ...
    internal static void OnSpawnableParameterChanged(CVRSpawnable spawnable, CVRSpawnableValue spawnableValue)
    {
        ...
        if (cache.ContainsKey(spawnableValue.name) &&
            Mathf.Approximately(cache[spawnableValue.name], spawnableValue.currentValue)) return;
    ...
    internal static void OnSpawnableParameterSet(string spawnableInstanceId, string spawnableParamName,
        float spawnableParamValue)
    {
        if (!PropCache.ContainsKey(spawnableInstanceId) || spawnableParamName == "" ||
            spawnableParamValue.IsAbsurd()) return;
        ...
        int? subIndex = null)
    {
        if (_debugMode)
            MelonLogger.Msg(
                $"[Debug] Attempting to set {spawnableInstanceId} [{(subIndex.HasValue ? subIndex.Value : "Main")}] location...");

        if (!PropCache.ContainsKey(spawnableInstanceId) || pos.IsAbsurd() || pos.IsBad() || rot.IsAbsurd() ||
            rot.IsBad())
        {
            if (_debugMode)
            {
        ...
        if (_debugMode)
            MelonLogger.Msg(
                $"[Debug] \t{spawnableInstanceId} [{(subIndex.HasValue ? subIndex.Value : "Main")}] location set!");
    }
    ...
    internal static void OnSpawnableCreate(string propGuid, float? posX = null, float? posY = null, float? posZ = null)
    {
        ...
        {
            if (posX.HasValue && posX.Value.IsAbsurd() && posY.HasValue && posY.Value.IsAbsurd() && posZ.HasValue &&
                posZ.Value.IsAbsurd())
            {
                // Spawn prop with the local coordinates provided
                PlayerSetup.Instance.SpawnProp(propGuid, new Vector3(posX.Value, posY.Value, posZ.Value));
            }
            else
            {
