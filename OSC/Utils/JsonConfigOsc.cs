...
namespace Kafe.OSC.Utils
{
    public static class JsonConfigOsc
    {
        ...
        public static void CreateConfig(string userGuid, string avatarGuid, string avatarName,
            AvatarAnimatorManager animatorManager)
        {
            try
            {
                ...
                {
                    // Ignore triggers because the configs don't support them
                    if (parameter.type == AnimatorControllerParameterType.Trigger) continue;

                    var input = new JsonConfigParameterEntry
                    {
                        address = AddressPrefixAvatarParametersLegacy + parameter.name,
                        type = parameter.type,
                    };
        ...
        public static JsonConfigAvatar GetConfig(string userGuid, string avatarGuid)
        {
            try
            {
                ...
                var currentAvatarConfig =
                    JsonConvert.DeserializeObject<JsonConfigAvatar>(File.ReadAllText(file.FullName));
        ...
        public static JsonConfigParameterEntry GetJsonConfigParameterEntry(string name, object value)
        {
            if (Converters.GetParameterType(value).HasValue)
            {
                ...
                {
                    address = AddressPrefixAvatarParametersLegacy + name,
